...
use rs_matter::dm::SubscriptionCtx;
...
fn test_client_subscribe_sender_non_chunked() {
    ...
    block_on(
        select(im.run(handler), async {
            ...
            assert!(established.matches(&SubscriptionCtx {
                fabric_idx: NonZeroU8::new(1).unwrap(),
                peer_node_id: remote_peer_id(&im),
                subscription_id: Some(established.subscription_id),
            }));
            assert!(
